...
by one or more lowercase English letters. For example, "/leetcode"
...
vector<string> splitStrDelim(string& s, string delimited){
    vector<string> res;
    size_t pos = 0;
    string token;
    while ((pos = s.find(delimiter)) != std::string::npos) {
        res.emplace(s.substr(0, pos));
