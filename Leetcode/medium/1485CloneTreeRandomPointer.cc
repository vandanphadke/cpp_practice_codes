...
private:
    unordered_map<Node*, NodeCopy*> nodesMap;
public:
    NodeCopy* copyRandomBinaryTree(Node* root) {
        return deepCopyRecurse(root);
    }

    NodeCopy* deepCopyRecurse(Node* root){
        if (root == NULL) {
            return NULL;
        }
        if (nodesMap.find(root) != nodesMap.end()) {
            return nodesMap[root];
        }

        NodeCopy* newroot= new NodeCopy(root->val);
        mmap[root] = newroot;

        newroot->left = deepCopyRecurse(root->left);
        newroot->right = deepCopyRecurse(root->right);
        newroot->random = deepCopyRecurse(root->random);
